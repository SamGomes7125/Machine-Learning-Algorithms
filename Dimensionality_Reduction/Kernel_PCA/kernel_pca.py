...
# -*- coding: utf-8 -*-
"""
Kernel Principal Component Analysis (Kernel PCA) - Dimensionality Reduction
"""
...
# Feature Scaling (Required for Kernel PCA)
sc = StandardScaler()
X_train = sc.fit_transform(X_train)
X_test = sc.transform(X_test)

# Applying Kernel PCA - Choosing top 2 principal components
kpca = KernelPCA(n_components=2, kernel="rbf")  # Using RBF kernel for non-linearity
X_train_kpca = kpca.fit_transform(X_train)
X_test_kpca = kpca.transform(X_test)

# Training Logistic Regression on Kernel PCA-transformed data
...
print(f"Model Accuracy after Kernel PCA: {accuracy * 100:.2f}%")
...
plt.title('Confusion Matrix (After Kernel PCA)')
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.show()

# Visualizing the Kernel PCA results (Training Set)
plt.figure(figsize=(7,5))
plt.scatter(X_train_kpca[:, 0], X_train_kpca[:, 1], c=y_train, cmap="coolwarm", edgecolor="k")
plt.title("Kernel PCA Applied to Wine Dataset (Training Set)")
